...
int check_value(std::string value, std::string controler)
{
	...
	if (value.find("-") != std::string::npos)
		return (-1);
	if (endptr[0] != '\0' || endptr == value.data() || val > INT_MAX || (controler == "val" && val > 1000))
		return (-1);
	return (0);
}
...
int	check_date(std::string date)
{
	...
	//? check if the form of the date is correct ?
	...
	//? check if the year is leap or not to add one day to February
...
void	BitcoinExchange::parseInput()
{
	...
	//? fill the map container white the values
	fill_data_map();
	//? export the first and the last date
	it = data.end();
	small_date = data.begin()->first;
	--it;
	big_date = it->first;

	//? starting the inpute file work
	...
	while (getline(this->f_input, line))
	{
        ...
        /**
         *? check if | not found 
         *? or there is more than one space before and after the |
